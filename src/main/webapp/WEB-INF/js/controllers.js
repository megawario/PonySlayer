...
angular.module('ponyslayer.controllers', [ 'ponyslayer.services' ])

// controller for adventures
.controller('adventuresController', function($scope, psAPI) {
	this.adventures = {}
	
	this.loadAdventures = function(){
		psAPI.getAdventures().then((function(response) {
			this.adventures = response.data;
		}).bind(this));
	}
	
	this.removeAdventure = function(id){
		psAPI.removeAdventure(id).then((
				function(response){ this.loadAdventures(); }).bind(this),
				alert("not allowed"));
	};
...
.controller('formController', function($scope, psAPI) {
	this.error=[]
	this.adventure = {}
	this.update= function(adventure) {
		psAPI.addAdventure(adventure).then(
				function(response){
					alert(response);
				},
				(function(response){
					//response status has error code
					//response data has the data of the message.
					if(response.status==400){ //display error messages:
						for(err in response.data.errors){
							var errorMessage = response.data.errors[err].field + " : " + response.data.errors[err].message;
...
//	    if (current.getDate() >= 21) currentMonth = (currentMonth+1)%12; // change
//																			// month
//																			// by
//																			// one
//																			// if
...
//	    (function(response){
//		this.adventures=response.data;
//		// do to contraints on input field [time] starttiem and endtime must be
